...
import newsemc.com.awit.news.newsemcappdftl.interfaces.GongYiGongFaInter;
 ...
 */
public class GYGFImpl implements GongYiGongFaInter {
    private String TAG="GYGFImpl";
    private List<CraftInfo> craftInfoList;
    private FailRequest failRequestobj;
    private HttpResultListener httpResultListener;
    public GYGFImpl(HttpResultListener httpResultListener){
        this.httpResultListener=httpResultListener;
    }

    @Override
    public void getGongYiGongFaList(String ssid,String pageno, String pagesize) {
...
//        client.setTimeout(ValueConfig.TIME_OUT);
        String url = ValueConfig.PCAPP_URL + "inter/inter!getGygf.action";
        client.post(
                url,
                params,
                new AsyncHttpResponseHandler() {
                    ...
                    public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
                        ...
                        Log.i(TAG, "succ");
                        craftInfoList=new ArrayList<CraftInfo>();
                        JSONArray jsonArr;
                        ...
                        try {
                            ...
                            for (int i = 0; i < dataarr.length(); i++) {
                                ...
                                CraftInfo craftInfobj=new CraftInfo();
                                craftInfobj.setInfoimg(jsonObject.getString("infoimg"));
                                craftInfoList.add(craftInfobj);
                            }
                            httpResultListener.onResult(craftInfoList);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
