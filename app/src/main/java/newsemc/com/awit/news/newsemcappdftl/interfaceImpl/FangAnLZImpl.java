...
import newsemc.com.awit.news.newsemcappdftl.dao.PreceptInfo;
import newsemc.com.awit.news.newsemcappdftl.interfaces.FangAnLZInter;
 ...
 */
public class FangAnLZImpl implements FangAnLZInter {
    private String TAG="FangAnLZImpl";
    private List<PreceptInfo> preceptInfoList;
    private FailRequest failRequestobj;
    private HttpResultListener httpResultListener;
    public FangAnLZImpl(HttpResultListener httpResultListener){
        this.httpResultListener=httpResultListener;
    }

    @Override
    public void getFangAnLZList(String ssid,String pageno, String pagesize) {
...
//        client.setTimeout(ValueConfig.TIME_OUT);
        String url = ValueConfig.PCAPP_URL + "inter/inter!getFalz.action";
        client.post(
                url,
                params,
                new AsyncHttpResponseHandler() {
                    ...
                    public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
                        ...
                        Log.i(TAG, "succ");
                        preceptInfoList=new ArrayList<PreceptInfo>();
                        JSONArray jsonArr;
                        ...
                        try {
                            ...
                            if(Integer.parseInt(s) >0){
                                ...
                                for (int i = 0; i < dataarr.length(); i++) {
                                    ...
                                    PreceptInfo preceptInfobj = new PreceptInfo();
                                    preceptInfobj.setHuiyiname(jsonObject.getString("huiyiName"));
                                    preceptInfobj.setDanwei(jsonObject.getString("danwei"));
                                    preceptInfobj.setJiyao(jsonObject.getString("jiyao"));
                                    preceptInfobj.setHuiyitime(jsonObject.getString("huiyiTime"));
                                    preceptInfobj.setId(jsonObject.getString("id"));
                                    preceptInfoList.add(preceptInfobj);
                                }
                                httpResultListener.onResult(preceptInfoList);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
